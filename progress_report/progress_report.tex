...
\usepackage[top=2.5cm,bottom=2.5cm,inner=4cm,outer=2.5cm]{geometry}%twoside
\usepackage{pdfpages} \usepackage[nottoc,notlof,notlot,numbib]{tocbibind}
...
% Spacing before heading is flexible (except for extended abstract).
\usepackage{titlesec}
...
% Change spacing between caption and figure
...
\usepackage{siunitx} \usepackage{gensymb} \usepackage{enumitem} % enumerate format
\usepackage{tabularx} \usepackage{booktabs} \usepackage{adjustbox} \usepackage{makecell}
\usepackage[style=ieee,backend=biber]{biblatex} \addbibresource{../zotero.bib}

\usepackage[cache=false]{minted}\usepackage{lmodern} % code font

\setcounter{bottomnumber}{1}
\renewcommand\bottomfraction{10.0} % encourage floats at bottom of page
