...
\usepackage{minted}\usepackage{lmodern}
...
This progress report involves identification and definition of the core thesis problem;
creating a semi-autonomous wheelchair which prioritises user safety and independence.
...
sensors which could be used to perceive the wheelchairs environment.
...
next semester). This involves collection of a second dataset using the final sensors, retraining of
...
should not impede the users comfort or the wheelchairs manoeuvrability.
...
\footnotetext[2]{The Microsoft Azure Kinect DK has multiple operating modes which trade-off between FOV, operating range, and resolution. The \texttt{NFOV unbinned} mode was compared, which provides good trade-off between operating range and resolution.}
...
Object localisation is another core problem within this field, and involves identifying the location of objects within an image
as well as classifying them. Object localisation can be used on a semi-autonomous wheelchair to identify
...
Encoding is typically done using a pretrained model backbone, such as ResNet, which reduces the computational power required to train
the model.
...
This algorithm takes a reference path as an input, and outputs a local path which avoids collisions and minimises jerk (rate of change of acceleration).
...
For the compute element, a Nvidia Jetson Xavier NX will be used, due to compatibility with the Zed API
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{images/yolov5s.png}
...
an example of model output can be seen in \cref{fig:deeplab}. To improve the speed of the model, input frames are
downscaled to 960x540, which causes the lower segmentation resolution.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{images/deeplab.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{images/hybridnets_outdoor.png}
...
and a Lidar sensor simulated at the wheelchairs position. This code is below;
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{images/simulated_lidar.png}
...
the users desired direction, as seen below. \Cref{fig:vfh_controller} shows a visualisation
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/vfh_controller.png}
