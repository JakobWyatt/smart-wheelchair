...
There has been greater interest in 
...
Other approaches to smart wheelchairs
in the past have utilized RGB-D stereo cameras \cite{jainAutomatedPerceptionSafe2014},
2d Lidar \cite{scudellariSelfdrivingWheelchairsDebut2017}, and ultrasonic sensors \cite{levineNavChairAssistiveWheelchair1999}
to percieve the surrounding environment. Self-driving cars built by companies such as Tesla and Waymo
use cameras, mmWave Radar, and 3d Lidar to avoid traffic and pedestrians.

Various sensor options are compared in \Cref{table:sensor_options}.
...
be used for different applications on the smart wheelchair - an inertial measurement unit (IMU) would help improve
wheelchair odometry, but was not listed below.
...
Several commercial options for an RGB-D camera are compared in \Cref{table:stereo_camera}.
Factors such as package size, field of view, and accuracy are important to consider due to the available
mounting points on the wheelchair. All of the below units come with an integrated IMU, and do not have a compute
element on board.
Costs are taken at the exchange rate of 1 AUD = 0.74 USD.

The Microsoft Azure Kinect DK has multiple operating modes which tradeoff between FOV, operating range,
and resolution. The configuration listed in the table below is the \texttt{NFOV unbinned} mode, which
provides good tradeoff between operating range and resolution.
...
\begin{adjustbox}{width=\textwidth}
    ...
    Microsoft Azure Kinect DK \cite{microsoftAzureKinectDK2021} & Active IR (ToF) & \$595 & $103\times 39\times 126$ & $75\degree\times 65\degree\times 75\degree$ & 0.5-3.86 \\
    \bottomrule
    \end{tabular}
\end{adjustbox}
...
% Compute element
The compute element  Note that the Stereolabs Zed products require an Nvidia GPU with CUDA capability to operate.
